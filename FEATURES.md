...
| Security foundation           | P0       | Shipped | Security module owner       | Environment validation, secure headers, origin/CORS policy, durable rate limits, auth security, tenant permissions, API key hashing, MFA policy, legal acceptance, privacy workflows, and security guidance exist.                          |
...
| Billing and payments          | P1       | Shipped | Billing module owner        | Provider abstraction, subscriptions, trials, invoices, entitlements, usage metering, idempotent signed webhooks, mock/Stripe-compatible adapters, admin/user UI, and webhook tests exist.                                                   |
| Currency and tax              | P1       | Shipped | Billing module owner        | Currency formatting/conversion abstractions, rounding rules, tax settings, VAT fields, manual tax provider abstraction, and invoice tax breakdown storage exist with compliance disclaimers.                                                |
| Storage and files             | P1       | Shipped | Storage module owner        | Provider adapters, signed URLs, validation, ownership checks, quotas, cleanup, previews, malware hook, and adapter tests are complete.                                                                                                      |
| Emails and notifications      | P1       | Shipped | Messaging module owner      | React Email templates, local preview, provider adapters, queueing, retries, delivery logs, localization, branding, and preferences are complete.                                                                                            |
| Public API and mobile support | P1       | Shipped | API module owner            | Versioned API, validation, standard errors, pagination/filtering/sorting, token auth, scopes, CORS, OpenAPI, SDK generation, OAuth adapters, mobile sessions/devices, upload intents, realtime events, usage tracking, and API tests exist. |
| Observability                 | P1       | Shipped | Observability module owner  | Structured logs, metrics, tracing, health/readiness checks, audit logs, request logs, job logs, and monitoring examples are complete.                                                                                                       |
